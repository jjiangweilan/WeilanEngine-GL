...
void GameEditor::render(void **data)
{
	...
	ImGui::End();
	//update camera
	auto camera = Camera::GetMainCamera()->GetEntity();
...
{
	if (Settings::gameDimension == Settings::GameDimension::Two)
		return;
	static int x, y;
	...
	if (mousePressingOnScene(dummyX, dummyY) && Input::isMouseClicked(MouseButton::Left))
	{
		...
		for (auto model : Model::collection)
		{
			...
			auto transform = model->GetEntity()->GetComponent<Transform>();
			...
			{
				minDis = distance;
				setSelectedGameObject(model->GetEntity());
			}
		}
	}
...
void GameEditor::showTRigidbodyInfo(Entity *entity)
{
	...
	{
		trb->sensor = sensor;
		auto &goJson = scene->sceneData.getData(trb->GetEntity());
	...
	if (selectedTRigidbody)
	{
		...
		{
			auto goTrans = selectedTRigidbody->GetEntity()->GetComponent<Transform>();
...
bool GameEditor::mousePressingOnScene(int &x, int &y, bool world, int mouse)
{
	...
	auto cameraPos = Camera::GetMainCamera()->GetEntity()->GetComponent<Transform>()->position;
	bool onWindow = false;
