...
void NpcSystem::Update()
{
	...
	if (Time::dayTimeInSecond > Time::dayDuration)
	{
		...
		{
			if (!npcController->GetEntity()->IsEnable() || npcController->GetEntity()->GetScene() != currentScene)
				continue;
			float remaining = 1;
			float random = 0;
...
void NpcSystem::move(NpcController *npcController)
{
		...
		Time::dayTimeInSecond < movementTo.from)
	{
		...
		auto transform = npcController->GetEntity()->GetComponent<Transform>();
		transform->setPosition({newPos.x, newPos.y, transform->position.z});
		auto animation = npcController->GetEntity()->GetComponent<Animation>();
