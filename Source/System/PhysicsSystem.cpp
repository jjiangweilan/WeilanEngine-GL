...
void PhysicsSystem::Update()
{
	/*for (auto& r : RigidBody::collection) {
			auto transform = r->GetEntity()->getComponent<Transform>();
	...
	{
		if (!body->GetEntity()->IsEnable() || body->GetEntity()->GetScene() != scene)
			continue;
									 ...
									 glm::vec2 &penetrationVec)
{
	...
	Transform *t1 = polygon1->GetEntity()->GetComponent<Transform>();
	Transform *t2 = polygon2->GetEntity()->GetComponent<Transform>();
									...
									glm::vec2 &penetrationVec)
{
	...
	Transform *t1 = polygon->GetEntity()->GetComponent<Transform>();
	Transform *t2 = circle->GetEntity()->GetComponent<Transform>();
...
bool PhysicsSystem::circleVsCircle(TRigidbody *circle1, TRigidbody *circle2,
								   glm::vec2 &penetrationVec)
{
	auto t1 = circle1->GetEntity()->GetComponent<Transform>();
	auto t2 = circle2->GetEntity()->GetComponent<Transform>();
							   ...
							   glm::vec2 &penetrationVec)
{
	...
	auto points1 = shape1->getPoints();
	auto points2 = shape2->getPoints();
	Transform *t1 = line1->GetEntity()->GetComponent<Transform>();
	Transform *t2 = line2->GetEntity()->GetComponent<Transform>();
	glm::vec2 pos1 = t1->position;
	glm::vec2 pos2 = t2->position;
								  ...
								  glm::vec2 &penetrationVec)
{
	...
	auto points1 = shape1->getPoints();
	auto points2 = shape2->getPoints();
	Transform *t1 = line->GetEntity()->GetComponent<Transform>();
	Transform *t2 = polygon->GetEntity()->GetComponent<Transform>();
	glm::vec2 pos1 = t1->position;
	glm::vec2 pos2 = t2->position;
								   ...
								   const bool &otherIsDynamic)
{
	...
	if (cr.separated || body->sensor || other->sensor || !body->GetEntity())
		return;
	if (otherIsDynamic)
	{
		cr.penetrationVec /= 2;
		body->GetEntity()->GetComponent<Transform>()->moveBy(cr.penetrationVec.x,
														cr.penetrationVec.y, 0);
		other->GetEntity()->GetComponent<Transform>()->moveBy(
