...
void NpcSystem::update() {
    ...
    if (Time::dayTimeInSecond > Time::dayDuration) {
        ...
        for (auto &npcController : NpcController::collection) {
            if (!npcController->entity->isEnable() || npcController->entity->getScene() != currentScene) continue;
            float remaining = 1;
            float random = 0;
...
void NpcSystem::move(NpcController *npcController) {
	...
	if (i == behavior.movement.size()) return;
	...
	else if (Time::dayTimeInSecond > movementTo.from)i++;
