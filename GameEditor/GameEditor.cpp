...
namespace wlEngine {
    ...
    void GameEditor::showGameObjectInfo() {
        ...
        if(selectedGameObject) {
            ...
            for (auto& c : go->components) {
				...
				else if (c->getId() == TRigidbody::componentId) showComponent(go, c.get(), "TRigidbody", std::bind(&GameEditor::showTRigidbodyInfo, this, std::placeholders::_1));
    ...
    bool GameEditor::mousePressingOnScene(int &x, int &y, bool world, int mouse) {
        ...
        bool onWindow = false;
        if (ImGui::IsMouseDown(mouse)) { // 0 left, 1 right, 2 mid
    ...
    void GameEditor::createTRigidbody() {
        ...
        if (ImGui::Button("Create")) {
            ...
            Shape* shape;
            if (selectedShape == shapeSelections[0]) shape = new PolygonShape({});
            else if (selectedShape == shapeSelections[1]) shape = new CircleShape({0,0}, 0);
            else shape = new LineShape({});
