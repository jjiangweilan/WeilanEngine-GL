...
namespace wlEngine {
    ...
    private:
        ...
        template <typename... Args> Json createComponentJson(const std::string &name, Args &&...);
        template <typename Arg> void createComponentJsonHelper(Json& params, int index, Arg &&);
        template <typename T, typename... Args> void createComponentJsonHelper(Json& params, int index, T&&, Args &&...);
    };
